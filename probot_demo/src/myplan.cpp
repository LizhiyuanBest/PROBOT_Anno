...
int main(int argc, char **argv)
{
    ...
    ros::Publisher  position_pub = node_handle.advertise<std_msgs::Float32MultiArray>("position_chatter",100);
    ...
    for(int k=0;k<mytgt_pose.size();k++){
        ...
        geometry_msgs::Pose pose_temp = mytgt_pose[k];
        pose1.push_back(pose_temp);
        
        // for(int step=0;step<3;step++)
        {
            // switch(step) // 去到一个位置用三步
            // {
            //     case 0: pose_temp.position.z-=45; pose1.push_back(pose_temp); break;
            //     case 1: pose_temp.position.z+=45; pose1.push_back(pose_temp); break;
            //     case 2: pose_temp.position.z-=45; pose1.push_back(pose_temp); break; 
            // }
            ...
            if(ret)//若求解成功
            {
                ...
                for(int i = 0;i<1;++i)
                {
                    ...
                    for (int j = 0; j < 6; ++j)
                    {
                        ...
                        switch(j)
                        {
                            case 0:postion_msg.data.push_back(sol_ang[i][j]*30) ;break;
                            case 1:postion_msg.data.push_back(sol_ang[i][j]*205/3) ;break;
                            case 2:postion_msg.data.push_back(sol_ang[i][j]*50) ;break;
                            case 3:postion_msg.data.push_back(sol_ang[i][j]*125/2) ;break;
                            case 4:postion_msg.data.push_back(sol_ang[i][j]*125/2) ;break;
                            case 5:sol_ang[i][j] = 0 /*angle*/;postion_msg.data.push_back(sol_ang[i][j]*200/9) ;break;//sol_ang[i][j] = angle; //sol_ang[i][j] = 0; 
                        }
                    }
                    postion_msg.data.push_back(1000);
    ...
    for (int j = 0; j < 6; ++j)
    {
        switch(j)
        {
            case 0:postion_msg.data.push_back(0.0) ;break;
            case 1:postion_msg.data.push_back(0.0) ;break;
            case 2:postion_msg.data.push_back(0.0) ;break;
            case 3:postion_msg.data.push_back(0.0) ;break;
            case 4:postion_msg.data.push_back(0.0) ;break;
            case 5:postion_msg.data.push_back(0.0) ;break;
        }
    }
    postion_msg.data.push_back(1000);
    position_pub.publish(postion_msg);
