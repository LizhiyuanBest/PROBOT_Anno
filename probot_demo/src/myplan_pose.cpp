...
void poseCallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
    ...
    model_pose.position.x = -0.12;
    model_pose.position.y = -0.347;
    model_pose.position.z = 0.21;
    model_pose.orientation.x = 1;
    ...
    if(!get_pose)
    {
        ...
        for(int i=0;i<match.size();i++)
        {
            ...
            target_position.position.x=model_pose.position.x + temp_thispose.T[0];
            target_position.position.y=model_pose.position.y + temp_thispose.T[1];
            target_position.position.z=model_pose.position.z; // + temp_thispose.T[2];
            target_position.orientation.x=temp_q.x();
            target_position.orientation.y=temp_q.y();
            target_position.orientation.z=temp_q.z();
            target_position.orientation.w=temp_q.w();
            pose_match.push_back(target_position);

            // std::cout<<"target_pose: "<<std::endl<<target_position<<std::endl;
        }
        get_pose = true;
    }
...
int main(int argc, char **argv)
{
    ...
    // target_pose.position.y = -0.347;
    // target_pose.position.z = 0.21;
    // target_pose.orientation.x = cos(atan2(abs(target_pose.position.y),abs(target_pose.position.x)));
    // target_pose.orientation.y = cos(atan2(abs(target_pose.position.y),abs(target_pose.position.x)));
    // mytgt_pose.push_back(target_pose);
